...
int flagExecutar = 0;
int estado = 0;   //flag de ativação do LDR
...
int cont = 0;   // contador para a abertura automatizada do portão
int off =0;     //flag para desligar tudo apos a contagem
int bound =0;
char resp=0;
...
void executarProtocolo(void) {
  ...
  if(ID[0] == 'M') { //O ID deste módulo é M01
  ...
  else if(EX[0] == 'D') { // O EX começa com D no protocolo de dimerização
    if(EX[1] == 'G') {
    ...
    if(EX[1] == 'H'){    // indica o alarme A que representa a Barreira infravermelho
